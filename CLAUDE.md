...
**POVINNÉ na začátku KAŽDÉ session, která se dotkne thesis — i po `/compact`, i po resume z předchozí konverzace, i když summary tvrdí že jsi soubory už načetl.** Po compaction zůstávají v kontextu jen fragmenty, ne celé kapitoly. Bez kompletního kontextu vznikají broken refs, terminologické konflikty, duplikáty argumentů a leakování metodiky do teorie.

Při `full-pass`, širší revizi, thesis auditu nebo jakékoli editaci v `thesis/` načti do hlavního kontextu:

1. `handoffs/29-kap02-rewrite-blueprint.md` (aktivní driver pro kap02 rewrite)
2. `thesis/prace.tex`
3. `thesis/uvod.tex`, `thesis/kap01.tex` až `thesis/kap05.tex`, `thesis/zaver.tex`
4. `notes/jak-psat-vedecky.md`
5. `notes/doporuceni-fis.md`
6. `notes/prirucka-craft-of-research.md`
7. `thesis/TERMINOLOGIE.md`
8. `thesis/makra.tex`

Trvalá full-pass pravidla (chapter contracts, phases, validation gates) žijí v memory `thesis_full_pass_rules.md`. Historické handoffy v `handoffs/archive/`.

**Triggery pro re-load:**
- Hláška `<system-reminder>` říkající že soubor "was read before the last conversation was summarized" → soubor v kontextu NEMÁŠ, načti znovu
- Resume z předchozí session přes summary → načti znovu, summary nestačí
- Po `/compact` → načti znovu
- Pokud při psaní nevíš odpověď na "co tenhle pojem říká kap03/kap04?" → načti znovu, ne hádej
