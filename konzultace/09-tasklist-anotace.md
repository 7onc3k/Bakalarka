...
## Workflow (revize 2026-05-07: lineárně po PDF)

DAG/Units přístup opuštěn — pro 14 sub-issues over-engineering. Postup:

- **Lineárně po stránkách PDF**, jeden sub-issue po druhém
- C1 (citace) hotové globálně skript passem (commit fe5f760)
- Strukturální přesuny v kap03 hotové (commit 9f5e5f0): Setup před metriky, Procedura běh, K3-44b zdrojů z 3.3 do kap02
- C6 (terminologie) jako závěrečný polish pass
- S* (strategická rozhodnutí) řeš v kontextu kapitoly, kde anotace leží — žádný blokující rozhodovací předstup
- Per-issue: audit (subagent) → diskuse → edit → commit (`Closes #NN`) → close

Stav S*: S1 #53 ✅, S4 #56 ✅; S2 #54 + S3 #55 open (rozhodnout v rámci L-Kap03).

Vrstvy 0–4 níže = **katalog** anotací (audit trail s plnými citacemi).

<details><summary>Historický DAG (superseded)</summary>

Původní plán Unit 0 (rozhodnutí) → Unit 1 (struktura) → Unit 2 (kap03) →
Units 3–7. Hotovo: Unit 0 částečně (S1, S4), Unit 1 commit 9f5e5f0.
Zbytek nahrazen lineárním postupem výše.

</details>

## Vrstvy — katalog anotací (zachováno jako audit trail)
