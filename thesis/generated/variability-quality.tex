...
\end{axis}
\end{tikzpicture}
\caption{Míra splnění operačních prahů \mgrp{Q} napříč devíti běhy
(\acs{Q2}~$\geq$~37, \acs{Q3}~$\geq$~70\,\%, \acs{Q4}~$\geq$~24,
\acs{Q5}~$\leq$~1, \acs{Q7}~$\leq$~1; viz sekce~\ref{sec:pilotni-iterace}).
Striktní exit z~tabulky~\ref{tab:metriky-prehled} (\acs{Q2}~=~42/42,
\acs{Q5}~=~0, \acs{Q7}~=~0~porušení) nesplnil žádný běh.
\acs{Q1} a~\acs{Q6} byly splněny ve všech bězích;
\acs{Q3} a~\acs{Q5} nejméně často.
\acs{Q3} počítáno ze~7 běhů (2~bez dat).
Metriky efektivity (\acs{E1}--\acs{E3}) nemají exit kritérium a~nejsou zahrnuty.}
