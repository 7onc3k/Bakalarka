...
\begin{raw}
\textbf{Audit trail (úvod, předchozí verze v2):}
4 odstavce: kontext + gap (benchmarky jen pass/fail), instrukce jako
problém, co práce dělá, průvodce po kapitolách. Zdroje tvrzení:
benchmarky jen pass/fail potvrzeno z kap02 sekce 2.3.1 (SWE-bench,
SWE-EVO, FeatureBench, ACE-Bench). ``Pass $\neq$ merge'' z METR 2026,
citováno v kap02. Cíle odpovídají kap01 řádky 97--109.

\textbf{Audit trail (úvod, předchozí verze v1):}
3 odstavce: kontext (agenti + instrukce), co práce dělá (metriky + ablace),
průvodce po kapitolách (výčet kap 1--5). Průvodce nahrazen stručným
popisem logiky návaznosti.
\end{raw}
