...
\textbf{a) Strukturovanost $>$ čistý natural language:}
\par\noindent $\rightarrow$ \texttt{specine2025}: LLM často špatně interpretuje specifikaci (``specification misalignment'').
Explicitní zarovnání vnímané vs. zamýšlené specifikace zvedlo Pass@1 o 29.6\%.
\par\noindent $\rightarrow$ Důsledek: strukturované formáty (šablony, I/O příklady) redukují misinterpretaci.
...
\par\noindent $\rightarrow$ Důsledek: acceptance criteria mapovatelná na testy jsou efektivnější než slovní popis.
...
\textbf{d) RE metodologie aplikovaná na prompty:}
\par\noindent $\rightarrow$ \texttt{reprompt2025}: Aplikace IEEE 29148 RE standardu na optimalizaci promptů
zlepšila výstupy. Ukazuje že tradiční RE postupy (elicitation, specification, validation) jsou aplikovatelné i pro LLM.

\textbf{Syntéza -- optimální specifikace pro agenty kombinuje:}
\begin{enumerate}
    \item \textbf{Structured natural language} -- šablona s Function, Inputs/Outputs, Pre/Post conditions \cite[kap.~4.4]{sommerville2016}
    \item \textbf{I/O příklady} -- konkrétní vstupy a očekávané výstupy (redukuje misinterpretaci)
    \item \textbf{Acceptance criteria} -- BDD Given/When/Then, mapovatelné na testy \cite{ticoder2024}
    \item \textbf{Doménový slovník} -- jasné pojmy a definice z business domény \cite{domaincodegen2024}
\end{enumerate}
