...
Dva nedávné studie empiricky hodnotí efektivitu repository-level context files
a~agent Skills --- s~protichůdnými, ale komplementárními závěry.

\par\noindent $\rightarrow$ \texttt{gloaguen2025agentsmd} --- Evaluating AGENTS.md (ETH Zurich, 2025):
\par\noindent AGENTBENCH benchmark (138 instancí, 12 repos) + SWE-bench LITE (300 tasků).
\par\noindent 4 agenti (Claude Code, Codex, Qwen Code), 3 podmínky (no context, LLM-generated, developer-written).

Klíčové nálezy:
\begin{itemize}
    \item LLM-generované context files \textbf{snižují} úspěšnost ($-3\,\%$)
          a~zvyšují cost ($+20\,\%$+)
    \item Developer-written context files jen \textbf{marginálně} zlepšují ($+4\,\%$)
    \item Agenti instrukce \textbf{následují} (více testů, explorace),
          ale to nezlepšuje výsledky
    \item Context files jsou redundantní s~existující dokumentací
    \item Když se odstraní dokumentace, context files najednou pomáhají ($+2{,}7\,\%$)
\end{itemize}

\par\noindent $\rightarrow$ \texttt{skillsbench2025} --- SkillsBench (BenchFlow, 2025):
\par\noindent 84 tasků, 11 domén, 7 agent-model konfigurací, 7308 trajektorií.
\par\noindent 3 podmínky: no Skills, curated Skills, self-generated Skills.

Klíčové nálezy:
\begin{itemize}
    \item Curated Skills zlepšují pass rate o~$+16{,}2$pp průměrně
    \item Self-generated Skills mají nulový nebo negativní efekt ($-1{,}3$pp)
    \item 2--3 fokusované Skills moduly jsou optimální ($+18{,}6$pp),
          4+ modulů jen $+5{,}9$pp
    \item Vyčerpávající (``comprehensive'') dokumentace výkon zhoršuje ($-2{,}9$pp)
    \item Menší model + Skills může překonat větší model bez Skills
\end{itemize}

\textbf{Syntéza:} Oba papery se shodují na klíčovém principu:
\textit{zbytečný kontext škodí, fokusovaný pomáhá}. Gloaguen ukazuje
že popisné přehledy (``co kde je'') nepřidávají hodnotu. SkillsBench
ukazuje že procedurální instrukce (``jak řešit'') hodnotu přidávají,
ale jen pokud jsou fokusované a~modulární.

\textbf{Propojení s BP:}
Naše AGENTS.md instrukce jsou procedurální artefakt (workflow, constraints,
quality gates), ne popisný přehled codebase. Experiment měří zda tento
typ instrukcí překonává baseline zjištěný Gloaguenem.
\end{raw}
...
%% ==========================================================================
%% Poznámka: fine-tuning vs in-context learning / prompt design
%% ==========================================================================

\begin{raw}
\textbf{Poznámka k zařazení BP do širšího kontextu (fine-tuning vs prompt design):}

Dva přístupy jak přizpůsobit LLM konkrétnímu úkolu:
\begin{itemize}
    \item \textbf{Fine-tuning} --- trénování parametrů modelu na specifických datech.
          Permanentní, drahé (GPU, data), pomalé.
    \item \textbf{In-context learning (ICL)} --- instrukce a příklady v~kontextovém okně.
          Ephemeral, levné, okamžité. Omezené velikostí context window.
\end{itemize}

Trend: context windows rostou (4K $\to$ 128K $\to$ 1M+), modely jsou lepší v~ICL.
Čím větší kontext, tím méně fine-tuningu je potřeba.
Lester et al. (2021) zavedli \textit{prompt tuning} --- trénování jen embeddingů promptu.

Naše BP se pohybuje v~prostoru ICL/prompt designu --- navrhujeme instrukce místo
trénování modelu. Otázka pro budoucí výzkum: pokud modely dosáhnou dostatečné
schopnosti učit se z~kontextu, bude fine-tuning pro řízení agentů vůbec potřeba?

\textbf{Souvislost s~evaluací:}
Existující benchmarky (SWE-bench) měří jen pass/fail výsledek --- ``vyřešil agent bug?''
Nikdo systematicky neměří \textit{proces} (jak agent pracoval) ani \textit{kvalitu}
výstupů beyond funkční korektnost. Naše práce tento gap adresuje evaluačním systémem
který měří proces (P1--P2), kvalitu (Q1--Q8) i~efektivitu (E1--E3).

\textbf{Existující nástroje pro prompt evaluation:}
Existují nástroje pro evaluaci a~iterativní zlepšování promptů ---
Langfuse, Braintrust, Promptfoo. Tyto nástroje měří kvalitu jednotlivých
odpovědí LLM (přesnost, latence, náklady) a~umožňují A/B testování promptů.
Nesplňují ale požadavky na evaluaci coding agentů:
\begin{itemize}
    \item Neměří proces napříč celou session (desítky zpráv, git historie)
    \item Nehodnotí výstupy verzování (commity, branches, pull requesty)
    \item Neměří kvalitu kódu (mutation score, lint, složitost)
    \item Nepokrývají procesní záznamy (kvalita commit messages, issue descriptions)
\end{itemize}
Jsou navržené pro konverzační AI (``odpověděl chatbot správně?''),
ne pro coding agenty (``vyvinul agent software správně a~správným postupem?'').

\textbf{Cline --- hill climbing na instrukcích:}
Praktický příklad iterativního zlepšování instrukcí popisuje tým Cline
(2025)\footnote{\url{https://cline.bot/blog/a-practical-guide-to-hill-climbing}}.
Iterativně zlepšovali system prompt svého coding agenta měřením
na benchmarku Terminal Bench (89 úloh): baseline $\to$ identifikace
chybových vzorců $\to$ úprava promptu $\to$ měření dopadu $\to$ opakování.
Dosáhli zlepšení z~47\,\% na 57\,\%. Tento přístup je totožný s~naším ---
ale není popsán akademicky a~chybí mu měření procesu (jen pass/fail).
\end{raw}
