...
% TODO(craft): Připomenout, že specifikace je součást experimentálního prostředí, ne hlavní přínos práce; relevance této sekce je v měřitelnosti chování agenta.
...
deterministickým rozhraním konzistentně vyhodnocovat referenční testy~\acs{Q2} i~mutation testing~\acs{Q3}.
...
Data jsou konzistentní s~hypotézou, že přepsání obecných pravidel na
konkrétní příkazy~\cite{breunig2025} a~jejich zasazení do pracovního
postupu jako kontrolní kroky je účinnější než ponechání stejného
požadavku v~obecné textové podobě. \acs{Q8}~=~3/3 ukazuje, že když musel
agent ověřit JSDoc před odevzdáním kódu, dokumentaci skutečně doplnil.
...
\texttt{transcript.json}; řádek \acs{E1} proto uvádí jen hodnoty pro
baseline r3 a~u~ablačních běhů zůstává n/a.
...
Tabulka ukazuje dva vzorce. Za prvé, pilotní fáze potvrzuje
že instrukce lze iterativně zlepšovat: r1 (baseline) splnila 4/10
tvrdých deterministických procesních a~produktových kritérií,
r3 (po dvou cyklech úprav) splnila 7/10.
...
Napříč všemi devíti běhy zůstaly stabilní zejména \ac{Q1}
a~\ac{Q6}~=~0. \ac{E3} na této úloze přinesla jen omezené
rozlišení. Nejvyšší variabilitu vykazovaly procesní metriky \acs{P2}--\acs{P5},
