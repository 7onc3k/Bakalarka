...
Then = postcondition) i behavioral model (přechody mezi stavy).
Tato redundance není žádoucí: pro člověka znamená vyšší cognitive overload,
pro LLM agenta zabírá vzácný context window duplicitními informacemi.

Otázkou je, která reprezentace je \textbf{nejefektivnější} --
diagramy jsou pro člověka ``high-bandwidth'' komunikace (rychlé pochopení
celkové struktury), ale LLM je zpracovávají jako text, ne vizuálně.
Konkrétní Given/When/Then scénáře mohou být pro agenta užitečnější
než abstraktní typové definice, nebo naopak.

Experimenty mohou ukázat optimální kombinaci elementů --
ne ``čím víc, tím lépe'', ale \textbf{která minimální sada}
reprezentací vede k nejlepším výsledkům.
