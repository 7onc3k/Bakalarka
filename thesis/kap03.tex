...
LLM-as-judge \cite{llmjudge2024} --- LLM hodnotí výstup agenta podle
fixního rubriku.

\textbf{Společný design:}
\begin{itemize}
    \item Škála 1--5 per dimenze
    \item Rubric (prompt + příklady per úroveň) fixní pro všechny běhy
    \item Validace: porovnání s~manuálním hodnocením autora na vzorku
\end{itemize}

\textbf{P5 --- procesní artefakty} hodnotí:
commit messages (popisnost, atomicita, konvenční prefix),
issue descriptions (scope, AC, kontext),
PR descriptions (co a~proč, odkaz na issue).

\textbf{Q8 --- kvalita kódu} hodnotí:
naming conventions, separation of concerns, idiomatický TypeScript,
...
[RAW] TODO: definovat přesné rubriky (prompt + příklady per škála)
pro P5 i~Q8
\end{raw}
