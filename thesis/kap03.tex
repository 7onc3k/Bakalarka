...
\begin{itemize}
    ...
    \item \textbf{Orchestrace} -- agent-internal orchestration. Agent dostane celý
          projekt a~sám rozhoduje o~dekompozici, pořadí, delegaci na subtasky.
          Vnější script slouží pouze jako spouštěč experimentu a~sběrač metrik.
...
\begin{itemize}
    ...
    \item Iterativní loop (Ralph Loop) -- agent běží v~cyklu s~čistým
          kontextem per iteraci; stav se předává přes soubory na disku
          (plán, git historie, issues). Řeší context degradaci bez nutnosti
          sub-agentů. Implementován v~Claude Code i~OpenCode jako
          built-in feature.
