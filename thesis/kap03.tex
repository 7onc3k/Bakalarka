...
\textbf{Meta-instrukce (experimentální proměnné):}

\begin{center}
\begin{tabular}{p{0.6cm}|p{4cm}|p{4cm}|p{3cm}}
\textbf{\#} & \textbf{Meta-instrukce} & \textbf{Očekávaný artefakt} &
\textbf{SDLC fáze} \\
\hline
M1 & Vytvoř si sub-agents pro různé role &
  \texttt{.opencode/agents/*.md} & Design \\
\hline
M2 & Navrhni architekturu před implementací &
  Design decisions v~issues & Design \\
\hline
M3 & Rozděl spec na sub-issues s~dependencies &
  GitHub Issues & Design \\
\hline
M4 & Definuj si workflow a~proces &
  Workflow v~AGENTS.md & Design \\
\hline
M5 & Nastav si quality nástroje &
  CI, linting, testing config & Implementation \\
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{p{0.7cm}|p{4cm}|p{7cm}}
\textbf{Run} & \textbf{Meta-instrukce} & \textbf{Co měříme} \\
\hline
R0 & M1+M2+M3+M4+M5 & Baseline -- plný meta-prompt \\
\hline
R1 & M2+M3+M4+M5 (bez M1) &
  Přínos sub-agent separace \cite{agentcoder2024, singleormulti2025} \\
\hline
R2 & M1+M3+M4+M5 (bez M2) &
  Přínos upfront architecture designu \\
\hline
R3 & M1+M2+M4+M5 (bez M3) &
  Přínos issue decomposition \cite{masai2024, wang2025cca} \\
\hline
R4 & M1+M2+M3+M5 (bez M4) &
  Přínos explicitního workflow \cite{hassan2025sase} \\
\hline
R5 & M1+M2+M3+M4 (bez M5) &
  Přínos quality tooling \\
\end{tabular}
\end{center}

Šest běhů, single run per podmínku. Každý běh probíhá v~separátním GitHub
repozitáři (čistý stav). Artefakty (commit history, issues, PR) zachovány
...
\begin{itemize}
    ...
    \item \textbf{Behavioral trace:} co agent reálně udělal -- vytvořil
          sub-agents? navrhl architekturu? nastavil CI? Ověřuje, že
          ablace meta-instrukce měla skutečný efekt na chování agenta.
\end{itemize}
