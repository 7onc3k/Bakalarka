...
\begin{raw}
[RAW] Tato sekce popisuje co agent dostane, co měříme, proč tyto metriky,
a referenční implementaci jako měřicí nástroj.
\end{raw}

\subsection{Specifikací řízený přístup}

\begin{raw}
[RAW] Obsah z bývalé sekce 3.2 (SDD) --- přesunut sem jako zdůvodnění
experimentálního setupu, NE jako výzkumná metodika.

Pro experiment je zvolen přístup kde specifikace řídí implementaci
(spec-first), na úrovni definované v~\cite{sdd2026}.

\textbf{Zdůvodnění volby pro experimentální design:}
\begin{enumerate}
    \item \textbf{Specifikace řídí implementaci} -- kvalita specifikace přímo ovlivňuje kvalitu výstupu agenta (viz 2.4.2 bod 7). Formalizace tohoto principu umožňuje kontrolovaný experiment.
    ...
    \item \textbf{Malé, focused úkoly} -- empirická data ukazují že malé agent PRs mají vyšší úspěšnost \cite{ehsani2026failedprs}. Spec-first přirozeně vede k dekompozici na testovatelné incrementy.
    \item \textbf{Spec-first stačí} -- pro jednorázový experiment není potřeba udržovat sync spec-kód. Spec se napíše, agent implementuje, vyhodnotí se.
\end{enumerate}
\end{raw}
...
\subsection{Experimentální dimenze a běhy}

\begin{raw}
[RAW] POZOR: Tento obsah popisuje faktoriální design R0--R5 z Posunu 7.
Design je ZASTARALÝ --- aktuální přístup je DSR iterativní pilot + ablace
(Posun 8, viz sekce~\ref{sec:vyzkumny-pristup}). Obsah potřebuje přepsat
na nový design. Zachováno pro referenci a přepracování.

\rule{\textwidth}{0.4pt}

Posun 8 — iterativní pilot + ablace/variace (2026-02-19)
...
\textit{ZASTARALÝ --- tento design bude nahrazen DSR iterativní pilot + ablace.}
...
\subsection{Katalog ablatable komponent}
