...
\begin{raw}
\textbf{REFRAME:} ``Cíle artefaktu'' $\to$ ``Sledované dimenze chování'' (nebo podobně).
Nezkoumáme artefakt (AGENTS.md), zkoumáme CHOVÁNÍ agenta.
AGENTS.md = nezávislá proměnná, chování = závislá proměnná.
``Cíle'' $\to$ ``dimenze v~kterých měříme vliv instrukcí na chování''.
Obsah (spec-first, TDD, kvalita kódu\ldots) zůstává --- jen jiný framing.
Pozor: na tuto sekci odkazují 3.5 exit kritéria i~kap04.

\medskip

V~terminologii DSR \cite{hevner2004} je artefaktem sada instrukcí
...
zjistili, že generické repository-level context files (popisné přehledy
codebase) mají marginální nebo negativní dopad na úspěšnost agentů
a~zvyšují inference cost o~více než 20\,\%. Naproti tomu Li et al.
\cite{skillsbench2025} ukazují, že fokusované procedurální instrukce
(2--3 modulární balíčky s~workflow a~příklady) zlepšují pass rate
o~16,2 procentního bodu. Oba výsledky podporují stejný princip:
\textit{minimální, cílený a~procedurální} kontext je efektivnější
než vyčerpávající dokumentace.

Cíle artefaktu proto reflektují procedurální charakter --- definují
\textit{jak} má agent pracovat (proces) a~\textit{co} má být
výsledkem (produkt), nikoliv popis repository.
\end{raw}
...
\begin{draft}
Metriky experimentu jsou organizovány podle taxonomie Fenton
...
\begin{description}
    \item[Q1 --- referenční test pass rate.] 40 behavioral testů
        ...
        \cite{mathews2024} (viz sekce~\ref{sec:referencni-testy}).
...
\begin{draft}
Metriky P2 (procesní artefakty), Q4 (AC coverage)
a~Q8 (kvalita kódu) hodnotí aspekty které nelze měřit čistě
deterministicky. Pro všechny používáme metodu LLM-as-judge
\cite{zheng2023mtbench} --- LLM hodnotí výstup agenta podle
fixního rubriku.
...
\begin{itemize}
    ...
    \item \textbf{Jeden prompt per metriku} --- P2 hodnotí tři dimenze
          procesních artefaktů, Q4 mapuje testy na acceptance criteria,
          Q8 hodnotí pět dimenzí kvality kódu --- každý s~vlastním
          rubrikem
...
\textbf{Q4 --- AC coverage:}
LLM dostane 24 acceptance criteria ze specifikace a~kód agentových
testů. Pro každé AC určí, zda existuje test který ho pokrývá.
Výstup: počet pokrytých AC / 24.
...
Q4 a~Q8 měří co agent \textit{vyrobil} (produktová dimenze).

Rubriky jsou definovány v~experimentální infrastruktuře
...
\label{sec:referencni-testy}

\begin{draft}
Metrika Q1 (funkční korektnost) vyžaduje sadu referenčních testů
...
\subsection{Přehled metrik a~nástrojů}
\label{sec:prehled-metrik}

\begin{draft}
Tabulka~\ref{tab:metriky-prehled} shrnuje metrický rámec.
Procesní metriky P1--P2 ověřují dodržování spec-first workflow a~TDD,
produktové Q1--Q8 pokrývají funkční korektnost, kvalitu testů
a~kvalitu kódu, efektivita E1--E3 slouží ke~srovnání mezi běhy.
Exit kritéria (prahové hodnoty) jsou v~sekci~\ref{sec:exit-kriteria}.

\begin{table}[H]
\centering
\caption{Metrický rámec: kód, měřená vlastnost a~nástroj měření}
\label{tab:metriky-prehled}
\small
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Kód} & \textbf{Metrika} & \textbf{Nástroj} \\
\midrule
\multicolumn{3}{@{}l}{\textit{Procesní (P) --- jak agent pracuje}} \\
P1 & Process compliance (6 položek) & git, GitHub API \\
P2 & Kvalita procesních artefaktů  & LLM-as-judge (GLM-5) \\
\midrule
\multicolumn{3}{@{}l}{\textit{Produktové (Q) --- co agent vyrobil}} \\
Q1 & Referenční test pass rate  & Vitest (40 behavioral testů) \\
Q2 & API contract match         & tsc (import + typecheck) \\
Q3 & Mutation score              & Stryker \\
Q4 & AC coverage (24 kritérií)   & LLM-as-judge (GLM-5) \\
Q5 & Lint warnings               & ESLint \\
Q6 & Typecheck errors            & tsc \texttt{-{}-noEmit} \\
Q7 & Cyklomatická složitost      & ESLint (complexity rule) \\
Q8 & Design quality              & LLM-as-judge (GLM-5) \\
\midrule
\multicolumn{3}{@{}l}{\textit{Efektivita (E) --- za jakou cenu}} \\
...
artefakt --- definuje pracovní postup, constraints a~quality gates.
...
\begin{raw}
\textbf{REFRAME:} ``odvozeno z~cíle artefaktu'' $\to$ ``odvozeno z~dimenzí chování'' + hlavička tabulek ``Cíl'' $\to$ ``Dimenze'' (oba výskyty níže).
\end{raw}

\begin{draft}
Exit kritéria definují minimální prahy pro přechod z~pilotní
...
sledování E1 (tokeny) jako kontrolního ukazatele --- instrukce
které zlepšují proces, ale dramaticky zvyšují spotřebu tokenů,
nemusí být v~praxi použitelné.
...
\begin{itemize}
    ...
    \item Li et al. \cite{skillsbench2025} --- 2--3 fokusované
          moduly produkují lepší výsledky než 4+ modulů (+18,6pp
          vs. +5,9pp). Vyčerpávající dokumentace výkon zhoršuje
          ($-2{,}9$pp). Ablace: testovat zda redukce instrukcí
          na jádro zlepší výsledky.
    \item Gloaguen et al. \cite{gloaguen2025agentsmd} --- agenti
          instrukce následují (více testů, explorace), ale
          zbytečné requirements zhoršují úspěšnost. Ablace:
          identifikovat které instrukce přidávají hodnotu
          a~které jen tokeny.
\end{itemize}
