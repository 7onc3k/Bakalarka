...
\begin{itemize}
    ...
    \item \textbf{Agent architektura} -- dvou-agentní design: (1)~orchestrátor
          (řízení flow + psaní testů z~AC) a~(2)~executor (implementace +
          verifikace). Separace testování od implementace je architektonická
          garance proti test oracle problému \cite{mathews2024} --
          oddělený agent na testy nemůže být ovlivněn implementačním kódem.
          AgentCoder \cite{agentcoder2024} empiricky validuje tento přístup
          (96,3\,\% HumanEval). Minimální hybridní architektura --
          multi-agent jen tam, kde je architektonický důvod
          \cite{singleormulti2025}.
\end{itemize}
...
\textbf{Ablační experimenty:}

Každý běh testuje konkrétní hypotézu z~literatury odebráním jedné nebo více
komponent oproti baseline (full spec + full scaffolding):

\begin{center}
\begin{tabular}{p{5cm}|p{3.5cm}|p{4cm}}
\textbf{Hypotéza (zdroj)} & \textbf{Co odebíráme} & \textbf{Co měříme} \\
\hline
Architektura ve spec je kritická \cite{specine2025} &
  Architecture vrstvu (types, invariants, behavioral model) &
  Dopad na correctness a~completeness \\
\hline
Scaffolding zlepšuje procesní kvalitu \cite{hassan2025sase} &
  Celý scaffolding (agents.md + CI + env) &
  Dopad na compliance a~conventions \\
\hline
TDD ze spec brání validaci bugů \cite{mathews2024} &
  TDD instrukce ze scaffoldingu &
  Kvalita testů (mutation score) \\
\hline
CI vynucování je nutné nad rámec písemných instrukcí &
  CI pipeline (ponechat agents.md) &
  Dodržování konvencí bez automatické kontroly \\
\end{tabular}
\end{center}

Baseline (full-scale autonomní běh s~plným scaffoldingem, bez lidského zásahu)
slouží jako referenční bod pro ablace. Referenční implementace (human-guided)
slouží jako gold standard pro metriky. Počet a~výběr ablačních běhů závisí
na dostupném scope -- prioritu mají hypotézy s~nejsilnější oporou v~literatuře.
