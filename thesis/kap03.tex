...
Experiment používá \textbf{2$\times$2 faktoriální design} se dvěma nezávislými
proměnnými (dimenzemi). Každá dimenze má dvě úrovně, což dává 4~experimentální
kombinace. Každá kombinace se spouští 2--3$\times$ pro zachycení variance.
Celkem 8--12 agent runs + 1~referenční implementace (human-guided baseline).
...
\textbf{Zdůvodnění sloučení LoopScript + MentorScript + AEE do jedné dimenze:}
...
commits'' a~CI pipeline to vynucuje. Separace by zdvojnásobila experimentální
matici bez jasného přínosu pro BP scope.

\textbf{Experimentální matice:}

\begin{center}
\begin{tabular}{l|c|c}
 & \textbf{Spec: Full} & \textbf{Spec: Minimal} \\
\hline
\textbf{Scaffolding: Full} & A: nejlepší podmínky & B: dobrý proces, vágní zadání \\
\textbf{Scaffolding: None} & C: přesné zadání, žádný guidance & D: nejhorší podmínky \\
\end{tabular}
\end{center}

Referenční implementace (human-guided) odpovídá kombinaci A (Full/Full)
a~slouží jako gold standard pro porovnání.
