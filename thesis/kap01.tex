...
\begin{raw}
\textbf{TODO --- přeformulovat cíle práce (poznámky 2026-03-16):}

Nová formulace cílů:
\begin{enumerate}
    \item \textbf{Literatura nemá procesní metriky pro agenty} --- existující benchmarky (SWE-bench aj.) měří jen výsledek. Navrhnout sadu metrik která pokrývá i~proces a~kvalitu. Zmínit toto explicitně jako research gap i~v kap02 (sec.\ hodnocení agentů).
    \item \textbf{Navrhnout instrukce aby agent docílil požadovaných výstupů} --- iterativní/pilotní fáze k~dosažení požadovaných cílů (exit kritéria).
    \item \textbf{Pomocí komparace (substituce a~ablace) zefektivnit výkon a~efektivitu agenta} --- komparativní fáze.
\end{enumerate}

Research question potřebuje přesná kritéria a~kvantifikaci --- ne vágní ``zlepšit'', ale měřitelné výstupy.
